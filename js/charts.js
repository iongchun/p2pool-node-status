...
var minerdraw= function(hashrate, doarate, payouts, blocks, renderTo, interval) {
  chart= new Highcharts.Chart({
    chart: {
      renderTo: renderTo,
      animation: true,
    ...
    tooltip: {
	  ...
	  if(point.series.name==='Your Shares') { val= (point.y).toFixed(8); } 

          s+= '<br/>' + point.series.name + ': ' + val;

        // if we display hash and doa, attach percentage to the doa rate
          if(point.series.name==='Dead on arrival')
	  {
	    doarate= point.y;
            if(hashrate > 0 && doarate > 0) {
              prop= ((doarate / hashrate) * 100).toFixed(2);
